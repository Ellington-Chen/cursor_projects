...
def _build_long_amount_frame(
    ...
    *,
    ...
    for value in df[amount_json_col].tolist():
...
def check_card_amount_diffs_different_institutions_refactored(
    ...
    *,
    ...
    if use_sampling:
        original_group_count = int(working_df.loc[:, sample_key_cols].drop_duplicates().shape[0])
    ...
    for amount_json_col in amount_json_cols:
        ...
        for _, group in long_df.groupby(group_keys, sort=False, dropna=False):
