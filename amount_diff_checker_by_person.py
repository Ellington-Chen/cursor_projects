...
def _build_long_amount_frame(
    ...
    *,
    ...
    dedupe_columns = _unique_columns([*group_keys, card_col, source_col, "institution", "value"])
...
def check_person_amount_diffs_different_institutions_refactored(
    df: pd.DataFrame,
    *,
    ...
    """
    按 idcard + dateBack 分组的版本：
    1. 默认按 idcard + dateBack 分组，同一人同一日期下不同卡号也会参与比较。
    2. 结果中 card 拆为 card1 / card2，记录匹配对各自的卡号。
    3. 使用长表 + 排序滑窗，只比较差值阈值内的候选。
    4. sample_seed 显式可配，便于复现。
    5. 支持进度可视化，并将 unique cards 单独导出为 CSV。
    """
    amount_json_cols = list(amount_json_cols)
    if not amount_json_cols:
        raise ValueError("amount_json_cols 不能为空")

    group_keys = list(group_keys) if group_keys is not None else [idcard_col, dateback_col]
    ...
    for amount_json_col in amount_col_iterable:
        ...
        if not column_result_df.empty:
            cards_set.update(column_result_df["card1"].dropna().astype(str).unique())
            cards_set.update(column_result_df["card2"].dropna().astype(str).unique())
        cards = sorted(cards_set)
...
def check_person_amount_diffs_different_institutions_fast(
    df: pd.DataFrame,
    ...
    results_df, summary = check_person_amount_diffs_different_institutions_refactored(
        df=df,
